...
async function up({ context: queryInterface }) {
  ...
  try {
    ...
    if (dbName === 'mysql') {
      // Add indexes
      await queryInterface.addIndex('gnrl_sub_regions', {
      ...
      await queryInterface.addIndex('gnrl_sub_regions', {
        ...
        name: 'idx_gnrl_sub_regions_name',
        transaction,
      });

      await queryInterface.addIndex('gnrl_sub_regions', {
        fields: ['status', 'deletedAt'],
        name: 'idx_gnrl_sub_regions_status',
        transaction,
      });
      await queryInterface.sequelize.query(
        `
          CREATE TRIGGER alter_gnrl_sub_regions_on_delete
            BEFORE UPDATE ON gnrl_sub_regions
            FOR EACH ROW
            BEGIN
      ...
      await queryInterface.sequelize.query(
        `
          ...
          BEGIN
            IF NOT EXISTS (
              SELECT 1 FROM pg_trigger WHERE tgname = 'trg_audit_logs_for_sub_regions'
            ) THEN
              CREATE TRIGGER trg_audit_logs_for_sub_regions
              AFTER INSERT OR UPDATE OR DELETE ON gnrl_sub_regions
              FOR EACH ROW
      ...
      await queryInterface.sequelize.query(
        `
        CREATE OR REPLACE FUNCTION alter_gnrl_sub_regions_on_delete_fn()
        RETURNS trigger AS $$
        BEGIN
      ...
      await queryInterface.sequelize.query(
        ...
        BEGIN
          IF NOT EXISTS (
            SELECT 1 FROM pg_trigger WHERE tgname = 'alter_gnrl_sub_regions_on_delete'
          ) THEN
            CREATE TRIGGER alter_gnrl_sub_regions_on_delete
            BEFORE UPDATE ON gnrl_sub_regions
            FOR EACH ROW
            WHEN (OLD."deletedAt" IS DISTINCT FROM NEW."deletedAt")
            EXECUTE FUNCTION alter_gnrl_sub_regions_on_delete_fn();
          END IF;
        END;
        $$;
          `,
...
async function down({ context: queryInterface }) {
  ...
  try {
    ...
    if (dbName === 'mysql') {
      ...
      await queryInterface.sequelize.query(
        `DROP TRIGGER IF EXISTS alter_gnrl_sub_regions_on_delete ON gnrl_sub_regions;`,
        {
          transaction,
        }
      );
      await queryInterface.sequelize.query(
        `DROP FUNCTION IF EXISTS alter_gnrl_sub_regions_on_delete_fn();`,
        {
          transaction,
        }
      );
    }
    await queryInterface.dropTable('gnrl_sub_regions', {
      transaction,
    });
    await transaction.commit();
  } catch (ex) {
