...
async function up({ context: queryInterface }) {
  ...
  try {
    ...
    if (dbName === 'mysql') {
      await queryInterface.addIndex('acl_resource_permissions', {
        fields: ['resourceID', 'permissionID', 'status', 'deletedAt'],
        unique: true,
...
async function down({ context: queryInterface }) {
  ...
  try {
    await queryInterface.removeIndex(
      'acl_resource_permissions',
      'idx_unique_acl_resource_permissions_fks',
      {
        transaction,
      }
    );
    await queryInterface.dropTable('acl_resource_permissions', {
      transaction,
    });
    await transaction.commit();
  } catch (ex) {
