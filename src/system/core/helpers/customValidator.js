...
class CustomValidator {
  ...
  registerExistsValidator() {
    Validator.registerAsync(
      'exists',
      async (value, attribute, param, passes) => {
        if (!attribute) {
          passes(
            false,
            'Specify Requirements i.e fieldName: exists:table,column'
          );
          return;
        }
  ...
  registerStrictValidator() {
    Validator.register(
      'strict',
      (value) => this.passwordRegex.test(value),
